...
const isBinaryFile = (name = '') =>
    /\.(jpe?g|png|gif|svg|webp|ico|bmp)$/i.test(name);
const isCodeFile = (name = '') =>
    /\.(html?|css|js)$/i.test(name);

function showPreview() {
    ui.body.classList.add('show-preview');
}

function showEditor() {
    ui.body.classList.remove('show-preview');
}
...
async function processFiles(fileList) {
    ...
    for (const file of fileList) {
        ...
        if (binary) {
            state.assetUrls[name] = URL.createObjectURL(file);
            state.files[name] = { file, text: undefined, isBinary: true };
        } else {
            state.files[name] = {
                file,
...
async function postImportSetup() {
    ...
    renderTree();
    const firstCode =
        Object.keys(state.files).find(k => k.endsWith('index.html')) ||
        Object.keys(state.files).filter(isCodeFile)[0];
    await openFile(firstCode);
    runPreview();
}

// FUNGSI RUNPREVIEW DENGAN RESPONSIF
function runPreview() {
    ...
    // Tambahkan meta viewport jika belum ada
    ...
    const pathReplacer = (match, attribute, pathValue) => {
        if (!pathValue || pathValue.startsWith('http') || pathValue.startsWith('data:') || pathValue.startsWith('//'))
            return match;
        const cleanPathValue = pathValue.replace(/^\.\//, '');
        let matchedAssetKey = null;
        for (const assetKey in state.activeAssetMap) {
            if (assetKey.endsWith(cleanPathValue)) {
                ...
                if (precedingCharIndex < 0 || assetKey[precedingCharIndex] === '/') {
                    matchedAssetKey = assetKey;
                    break;
                }
            }
        }
...
ui.desktopViewBtn.addEventListener('click', () => {
    ui.previewView.classList.replace('view-mobile', 'view-desktop');
    ui.desktopViewBtn.classList.add('active');
    ui.mobileViewBtn.classList.remove('active');
});

ui.mobileViewBtn.addEventListener('click', () => {
    ui.previewView.classList.replace('view-desktop', 'view-mobile');
    ui.mobileViewBtn.classList.add('active');
    ui.desktopViewBtn.classList.remove('active');
});
...
function clearAll() {
    ...
    ui.previewIframe.src = 'about:blank';
}
...
function renderTabs() {
    ...
    const codeFiles = Object.keys(state.files)
        .filter(isCodeFile)
        .sort((a, b) => {
            const o = { html: 1, css: 2, js: 3 };
            return (o[a.split('.').pop()] || 99) - (o[b.split('.').pop()] || 99);
        });
    codeFiles.forEach(path => {
        const btn = document.createElement('button');
        btn.className = `btn ${path === state.currentPath ? 'active' : ''}`;
        btn.onclick = () => openFile(path);
        const ext = path.split('.').pop();
        let i = 'fa-regular fa-file-code';
        if (ext === 'html') i = 'fa-brands fa-html5';
        if (ext === 'css') i = 'fa-brands fa-css3-alt';
        if (ext === 'js') i = 'fa-brands fa-js';
        ...
        btn.querySelector('.close-btn').onclick = (e) => {
            e.stopPropagation();
            console.log("Fungsi close belum diimplementasikan untuk:", path);
        };
...
function renderTree() {
    ...
    filePaths.forEach(path => {
        ...
        path.split('/').forEach((part, i, arr) => {
            ...
            if (i === arr.length - 1) current[part].__path = path;
    ...
    function createHtml(node) {
        ...
        const keys = Object.keys(node).sort((a, b) =>
            (node[a].__path ? 1 : -1) - (node[b].__path ? 1 : -1) || a.localeCompare(b)
        );
        keys.forEach(key => {
            ...
            if (data.__path) {
                ...
                let icon = isBinaryFile(key) ? 'fa-image' : 'fa-file-code';
                ...
                const nestedUl = createHtml(data);
                nestedUl.classList.add('nested', 'collapsed');
                li.append(folderDiv, nestedUl);
                folderDiv.onclick = () => {
                    folderDiv.querySelector('.folder-toggle').classList.toggle('expanded');
...
function ensureDefaultFiles() {
    ...
    state.files['index.html'] = {
        text: '<h1>Selamat Datang!</h1>\n<p>Klik <i class="fa-solid fa-play"></i> untuk preview.</p>\n<link rel="stylesheet" href="style.css">\n<script src="script.js"></script>',
        isBinary: false
    };
    state.files['style.css'] = {
        text: 'body { font-family: sans-serif; text-align: center; margin-top: 50px; background-color: #f0f8ff; }',
        isBinary: false
    };
    state.files['script.js'] = {
        text: 'console.log("Selamat datang!");',
        isBinary: false
    };
}
