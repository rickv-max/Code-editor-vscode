...
async function processFiles(fileList) {
    ...
    for (const file of fileList) { 
        ...
        if (binary) { 
            ...
            state.files[name] = { file, text: await file.text(), isBinary: false }; 
        } 
    }
...
// === Perbaikan runPreview + Meta Viewport ===
function runPreview() {
...
// === Fungsi untuk scaling iframe di mobile view ===
function fitIframeToMobile() {
    if (!ui.previewView.classList.contains('view-mobile')) return;

    const screen = document.querySelector('#preview-view.view-mobile .device-screen');
    const iframe = ui.previewIframe;

    const screenWidth = screen.clientWidth;
    const screenHeight = screen.clientHeight;

    const contentWidth = 375; // ukuran iPhone X
    const contentHeight = 812;
...
ui.desktopViewBtn.addEventListener('click', () => { 
    ...
    ui.previewIframe.style.transform = '';
    ui.previewIframe.style.width = '100%';
    ui.previewIframe.style.height = '100%';
});
...
window.addEventListener('resize', fitIframeToMobile);
...
function renderTabs(){
    ...
    const codeFiles=Object.keys(state.files).filter(isCodeFile).sort((a,b)=>{
